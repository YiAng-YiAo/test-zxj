...
function setGlobalTimer()
	...
	local open_server_day_zerotime = System.getOpenServerStartDateTime()
	if System.getNowTime() < open_server_day_zerotime then
		--没开服，不处理，直到onNewDay
		System.log("guildbattle", "not setGlobalTimer", System.getNowTime(),open_server_day_zerotime)
		return
	end
...
function OnGuildBattleNewDay()
	if System.getOpenServerDay() == 0 then
		setGlobalTimer()
	end
end
initGlobalData()
...
engineevent.regNewDay(OnGuildBattleNewDay)
------------------------------------------------
