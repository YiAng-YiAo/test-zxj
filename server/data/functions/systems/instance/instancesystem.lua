...
local function onCanEnter(hdl, actor)
	local var = LActor.getDynamicVar(actor)
	if var.startClientPk then
		LActor.sendTipmsg(actor, Lang.ScriptTips.f00076, ttHearsay )
		return false
	end
	return  true
end
...
_G.onInstanceCanEnter = onCanEnter
...
local function onReqExit(actor, packet)
	...
	local var = LActor.getDynamicVar(actor)
	if var.ChallengeBossFlag ==  true then
		var.ChallengeBossX = x
		var.ChallengeBossY = y
		var.ChallengeScene = LActor.getSceneId(actor)
	end
