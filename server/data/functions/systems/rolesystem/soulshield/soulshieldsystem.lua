...
function soulshieldStageup(actor,roleId,type)
	...
	soulshieldTakeOnOff(actor,roleId,type,1,stage) --升阶直接上马
...
--takeOnFlag 1上马 0下马
--act 0未激活 正数：当前上马。负数:上次上马。数值从1开始(对应0阶).3就是当前使用2阶，-4就是当前下马，上次使用的是3阶
--takeOnStage 要上马对应的阶数
function soulshieldTakeOnOff(actor, roleId, type, takeOnFlag, takeOnStage)
	if type ~= ssLoongSoul then return end--暂时只考虑龙魂(宝物)
	local stage,level,exp,act = LActor.getSoulShieldinfo(actor, roleId, type)
	if act == 0 then
		return --没激活
	end
	takeOnStage = takeOnStage or stage
 	
	if takeOnStage < 0 or takeOnStage > stage then
		return
	end
	if takeOnFlag == 1  then--上马
		act = takeOnStage + 1
	end 
	if takeOnFlag == 0  then--下马
		if act > 0 then
			act = - (act)
		end
	end
	...
	--激活
	LActor.setSoulShieldAct(actor,roleId,type, act)
	if takeOnFlag == 1 then
		zhuangbansystem.UnUseOnePos(actor,roleId,zhuangbansystem.zhuangban_pos_soulshield)
	end
	local pack = LDataPack.allocPacket(actor, Protocol.CMD_SoulShield, Protocol.sSoulShieldCmd_TakeOnOffResult)
...
local function soulshieldAct(actor, roleId, type)
	if type ~= ssLoongSoul then return end--暂时只考虑龙魂(宝物)
	local stage,level,exp,act = LActor.getSoulShieldinfo(actor, roleId, type)
	if act ~= 0 then return end --已经激活过了
	--还没到激活等级
...
function soulshieldLevelup(actor, roleId, type)
	local stage,level,exp,act = LActor.getSoulShieldinfo(actor, roleId, type)
	if (not level or not stage or not exp or act == 0) then
		return
	end
	if soulshieldcommon.getOpenLv(type) > LActor.getLevel(actor) then
...
function soulshieldTakeOnOff_c2s(actor, pack)
	local roleId = LDataPack.readShort(pack)
	local type = LDataPack.readShort(pack)
	local takeOnFlag = LDataPack.readByte(pack)
	local takeOnStage = LDataPack.readByte(pack)
	soulshieldTakeOnOff(actor, roleId, type, takeOnFlag, takeOnStage)
end
...
netmsgdispatcher.reg(Protocol.CMD_SoulShield, Protocol.cSoulShieldCmd_TakeOnOff, soulshieldTakeOnOff_c2s)
