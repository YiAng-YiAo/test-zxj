...
local ZHUANGBAN_MAXPOS = 4
local ZHUANGBAN_MAXID = #ZhuangBanId
zhuangban_pos_soulshield = 4
...
function handleActive(actor, packet)
    ...
    LActor.costItem(actor, itemId, num, "激活时装")
...
local function handleUse(actor, packet)
    ...
    local pos1, pos2, pos3, pos4 = (v[1] or 0), (v[2] or 0), (v[3] or 0),(v[4] or 0)
    LActor.setZhuangBan(actor, roleindex, pos1, pos2, pos3, pos4)
    if zhuangban_pos_soulshield == conf.pos then
         soulshieldsystem.soulshieldTakeOnOff(actor, roleindex, ssLoongSoul, 0)
    end
...
function UnUseOnePos(actor, roleId, pos)
    if roleId < 0 or roleId > LActor.getRoleCount(actor)-1 then
        return
    end
    local var = getStaticVar(actor)
    var.use[roleId][pos] = 0
    LActor.log(actor, "zhuangbansystem.handleUnUse", "mark1", roleId, pos)
    local v = var.use[roleId]
    local pos1, pos2, pos3,pos4 = (v[1] or 0), (v[2] or 0), (v[3] or 0),(v[4] or 0)
    LActor.setZhuangBan(actor, roleId, pos1, pos2, pos3, pos4)
...
local function handleUnUse(actor, packet)
    local roleindex = LDataPack.readByte(packet)
    local id = LDataPack.readInt(packet)
    if not checkRoleIndexAndZhuangbanId(actor, roleindex, id) then
        return
    ...
    UnUseOnePos(actor, roleindex, conf.pos)
end
...
local function onInit(actor)
    ...
    for roleindex = 0, 2 do
        local v = var.use[roleindex]
        for posId =1,4 do--中间插入修改配置id，导致数据异常，要检查修正下20181031
            local useId = v[posId] or 0
            if ZhuangBanId[useId] and ZhuangBanId[useId].pos ~= posId then
                v[posId] = 0
            end
        end
        local pos1, pos2, pos3,pos4 = (v[1] or 0), (v[2] or 0), (v[3] or 0),(v[4] or 0)
