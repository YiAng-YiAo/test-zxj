...
gmCmdHandlers.opentime = function(actor,arg)
   System.setOpenServerTime(arg[1], arg[2], arg[3], arg[4], arg[5] or 0)
   return true
end
...
gmCmdHandlers.addrecharge = function(actor, arg)
    ...
    local paytype = arg[2] or "android"
    if yb >0 then
...
gmCmdHandlers.itemupdate = function()
	return System.itemUpdate() and System.reloadGlobalNpc(nil, 0)
end
...
gmCmdHandlers.circle = function(actor,args)
	local circle = tonumber(args[1]) or 0
    actorzhuansheng.gmCircle(actor, circle)
    return true
end
...
gmCmdHandlers.biandage=function(actor, args)
	...
	ad.zhuansheng_lv = #ZhuanShengLevelConfig - 1  --转生等级
	ad.essence = 100000
    ...
    for id,_ in ipairs(ZhuangBanId) do
    	var.zhuangban.zhuangban[id] = 7*24*3600 + System.getNowTime()
    	var.zhuangban.zhuangbanlevel[id] =  1
    end
    --战灵
...
gmCmdHandlers.printpro = function ( actor, args )
	local attrType = tonumber(args[1])
	local str = " "
	for i=0, LActor.getRoleCount(actor)-1 do
		local role = LActor.getRole(actor, i)
		if role then
			str = str ..LActor.getAttr(role,attrType).." "
		end
	end
	print(str)
	return true
...
gmCmdHandlers.clearqqvip = function ( actor, args )
    local var = LActor.getStaticVar(actor)
    var.qqvipgift = nil
end
...
gmCmdHandlers.runfun = function(actor,args)
	local funcName = args[1]
	loadstring(funcName)()
end
--@call module_name fun_name
--@call global_fun_name
gmCmdHandlers.call = function(actor, args)
	if type(args[1])~= "string" then
		return
	end
	local module_name = args[1]
	local m = package.loaded[module_name]
	if m and type(args[2]) == "string"  then
		local fun_name = args[2]
		table.remove(args,1)
		table.remove(args,1)
		m[fun_name](args)
		return true
	elseif _G[args[1]] then
		local fun_name = args[1]
		table.remove(args,1)
		_G[fun_name](args)
		return true
	end
end
gmCmdHandlers.act = function (actor, args)
	local id = tonumber(args[1])
	local endId = tonumber(args[2]) or id
	local ret = true
	for i=id,endId do
		if not activitysystem.gmStartActivity(actor, i) then
			ret = false
		else
			print("start ok ="..i)
		end
