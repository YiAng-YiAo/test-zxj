...
local function onResult(actor, packet)
    ...
    --实际奖励
    LActor.changeCurrency(actor, NumericType_Exp, exp, "遭遇战")
    LActor.changeCurrency(actor, NumericType_Gold, gold, "遭遇战")
    LActor.changeCurrency(actor, NumericType_Essence, essence, "遭遇战")
...
local function onReqDrop(actor, packet)
    ...
    LActor.giveAwards(actor, cache.rewards, "遭遇战")
    cache.rewards = nil
end
...
local function startClientPk(actor, packet)
	local var = LActor.getDynamicVar(actor)
	var.startClientPk = true
end
function onEndClientPk(actor)
	local var = LActor.getDynamicVar(actor)
	var.startClientPk = nil
end
...
netmsgdispatcher.reg(Protocol.CMD_Skirmish, Protocol.cFieldPlayerCmd_StartClientPk, startClientPk)
