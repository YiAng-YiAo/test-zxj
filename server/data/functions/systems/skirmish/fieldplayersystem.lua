...
local function createFieldPlayer(actor, maxNum, sp, ep, posCfg, conf)
	...
	for i = 1,maxNum do 
		if var.robot[i] then
			...
			table.insert(nameList, var.robot[i].name)
		end
	end
	--刷出玩家
	for i = sp,ep do
		...
		for k,aid in ipairs(ret) do 
			if inTable(aid, list) == false  then 
				local name = ""
				local sex = LActor.getActorSex(aid)
				for z = 1, maxNum do
					name = LActorMgr.getRandomName(sex)
					if not inTable(name, nameList) then
						break
					end
				end
				...
				LActor.createFieldPlayerData(actor, i, aid, posCfg[i][1], posCfg[i][2], actionType, isForay, conf.killNum or 0, ox, oy,name)
				break
			end
		end
	end
...
local function onResult(actor, packet)
	print("fieldplayersystem onResult ")
    ...
    if result == 1 then
		...
		--记录增加红名值
		--skirmish.changePkval(actor, nil, SkirmishBaseConfig.onesPkval)--暂时屏蔽
		--/************************/--
		var.robot[id] = nil
		--获取通关关数
		local level = LActor.getChapterLevel(actor)
