...
--后台设置运营活动 
function GmSetActivity(act_type,act_id, act_conf, act_content)
...
function gmStartActivity(actor, id)
	if id == 99999 then
		--999全部关闭
		for k,v in pairs(ActivityConfig) do
			if globalData.activities[k] then
				globalData.activities[k].endTime = System.getNowTime() - 1
				sendActivityData(actor, k)
			end
		end
		return true
	end
	...
	local actCfg = ActivityConfig[id]
    local type = actCfg.timeType
    if not type then
    	return false
    end
	...
	if not activityTimeIsEnd(id) then
		--已经开启关闭
		globalData.activities[id].endTime = System.getNowTime()-1
		sendActivityData(actor, id)
		return true
	end
    if type == 0 then
        System.setOpenServerTime(year, month, day, h, m)
        actCfg.startTime = "0-0:0"
		actCfg.endTime = "30-0:0"
    elseif type == 1 then
        actCfg.startTime = string.format("%d.%d.%d-0:0",year, month, day)
		actCfg.endTime = string.format("%d.%d.%d-0:0",year, month+1, day)
    elseif type == 2 then
 		actCfg.startTime = "0-0:0"
		actCfg.endTime = "30-0:0"
		...
		if cfg then
			cfg.time = string.format("%d.%d.%d-00:00:00",year, month, day) 
		end
 	else 
 		return false
    end

    actCfg.mergeCount = nil
    actCfg.idLimit = nil
  	actCfg.idOpenLimit = nil
  	actCfg.openTimeLt = nil
  	actCfg.openTimeGt = nil
  	actCfg.hefuTimeLt = nil
  	actCfg.hefuTimeGt = nil
  	actCfg.hfTimes = nil
  	actCfg.needCross = nil
  	onStart() 
    
    local type = ActivityConfig[id].activityType
    if subactivities.getConfig(type) then
        subactivities.init(type, id,globalData.activities[id])
    end
