		...
		for k, v in pairs(info.damagelist) do
			...
			if basePrecent <= percent and k ~= LActor.getActorId(boss.belong) then
				...
				if conf.dropType == 1 and actor and LActor.canGiveAwards(actor, reward) and ahfuben == boss.hfuben then
					LActor.giveAwards(actor, reward, "BOSS("..(ins.data.id)..")奖励")
				else
...
local function startLottery(ins, reward)
    ...
    boss.lottery.record = {}
    boss.lottery.hfuben = boss.hfuben
    --给护盾造成过伤害的玩家发送摇骰子界面
...
function sendBelongReward(ins)
	...
	if boss.belong then
		...
		System.writeLocalLog(qlBossBelong, boss.belong, id)
	end
	--奖励掉落
	...
	if LActor.getFubenHandle(boss.belong) == boss.hfuben then
		if conf.dropType == 1 then
			if LActor.canGiveAwards(boss.belong, reward) then
				LActor.giveAwards(boss.belong, reward, "BOSS("..(ins.data.id)..")奖励")
			else
			    --邮件
...
local function enter(actor, id)
    ...
    local boss = getBossData(id)
    if boss and boss.hfuben then
    	local ins = instancesystem.getInsByHdl(boss.hfuben)
		if ins then
			local sceneId = ins.scene_list[1] 
			if not Fuben.canMove(sceneId,x*GRIDSIZE,y*GRIDSIZE) then
				print(string.format("[Error]worldboss Enter pos Cant move bossId:%d,x:%d,y:%d",id,x,y))
				x = 0 
				y = 0
			end
		end
...
local function onReqLottery(actor, packet)
	...
	if nil == boss.lottery.hfuben then return end
	--判断玩家还在不在副本里面
	if boss.lottery.hfuben ~= LActor.getFubenHandle(actor) then
    ...
    if roll > boss.lottery.point then
        ...
        local ins = instancesystem.getInsByHdl(boss.lottery.hfuben)
        ...
        for k, v in pairs(boss.shieldDamageList) do
			...
			if oactor and boss.lottery.hfuben == LActor.getFubenHandle(oactor) then
...
local function doBuyDayCount(actor, boss_type)
	...
	if price > 0 then
		...
		System.writeLocalLog(qlBuyBossTimes,actor,boss_type,yb)
	end
...
function checkWorldBossEnterPoint()
	for k,v in pairs(WorldBossConfig) do
		local hFuben = Fuben.createFuBen(v.fbid)
	    local ins = instancesystem.getInsByHdl(hFuben)
		if ins then 
			local sceneHandle = ins.scene_list[1]
			for _,pos in ipairs(v.enterPos) do
				if not Fuben.canMove(sceneHandle,pos.posX*GRIDSIZE,pos.posY*GRIDSIZE) then
					log_error(string.format("[WorldBossConfig] Enter pos Cant move bossId:%d,x:%d,y:%d",v.bossId,pos.posX,pos.posY))
				end
			end
		end
...
engineevent.regGameStartEvent(checkWorldBossEnterPoint)
