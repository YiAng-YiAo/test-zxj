...
local function checkKnockoutOver()
	...
	local no_aid = -1 
	for i,v in ipairs(pr_num) do
		pr[v.i][no_aid] = 0 --轮空设置id<0,客户端没轮空数据会导致连线错误
		no_aid = no_aid -1
	end
...
local function sendSignUpDataAll()
	for i,actor in ipairs(System.getAllActorList() or {}) do
...
local function checkPromPkOver(st)
	...
	if gnum >= table.getnEx(promData) then
		...
		elseif st == Status.Finals then--决赛结束了
			...
			--给冠军发一个邮件
			if baseCfg.winMail and winAid and winAid > 0 then
		...
		sendSignUpDataAll()--跨服显示时间计算依赖curStarTtime
		sendAllStatusChange(nil)
		if EidTable[st] then
	...
	for idx,pinfo in ipairs(promData) do
		if not promWinData[idx] then
			...
			elseif #aids == 1 then
				...
				if aids[1] > 0 and aids[2] > 0 then--轮空id < 0
					--创建一个副本
					local data = {}
					data.pinfo = pinfo
					data.st = st
					...
					promWinData[idx] = math.max(aids[1],aids[2])
					hasOnes = true
				end
			end
		end
