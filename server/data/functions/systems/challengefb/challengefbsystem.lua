...
local function onFbWin(ins)
	...
	--log
	System.writeLocalLog(qlChallengefbWin,actor, (var.curId or 0))
end
...
local function reqLottery(actor, packet)
	...
	--log
	System.writeLocalLog(qlFbLottery,actor, randI, var.lotteryCount)   
	--发送抽奖结果给客户端
...
local function onRecReward(actor)
	...
	System.writeLocalLog(qlChallengefbReward,actor, (var.dayRewardsRec or 0))
	--发一次奖励界面信息
	onGetReward(actor)
	challengeInfoSync(actor)
end
