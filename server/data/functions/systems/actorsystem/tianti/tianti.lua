...
local function buyChallengesCount(actor)
	...
	System.writeLocalLog(qlBuyChallenges,actor, var.buy_challenges_count,  TianTiConstConfig.maxRestoreChallengesCount - var.challenges_count)
	if var.challenges_count   >= TianTiConstConfig.maxRestoreChallengesCount then 
		var.challenges_count_cd = 0
		var.challenges_count_cd_time = os.time()
	end
...
local function onMatchingActor(actor,packet)
	...
	elseif actor_id ~= 0 then --匹配玩家成功
		...
		local conf = TianTiDanConfig[basic_data.tianti_level][basic_data.tianti_id]
		LDataPack.writeInt(npack,conf.showDan)
		var.is_matching = {}
	...
	if var.is_matching ~= nil then 
		...
		LActor.sendTipmsg(actor, Lang.ScriptTips.tianti001, ttHearsay)
	end
end
...
function getTiantiBeginId()
	for i,v in pairs(TianTiDanConfig[TianTiConstConfig.beginLevel]) do
		if v.showDan == TianTiConstConfig.beginShowDan then
			return v.id
		end
	end
	return 0
...
_G.getTiantiBeginId = getTiantiBeginId
