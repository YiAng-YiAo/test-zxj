...
local function onReqRewards(actor, packet)
    ...
    LActor.giveAwards(actor, rewards, "vip奖励")
end
...
local function onLogin(actor)
    ...
    if pf == "ayx" or pf == "ay1" then
        if vipLevel < 1 then
            LActor.setVipLevel(actor,1)
            updateVipAttr(actor)
        end
    end
    --自动成vip
    for i,v in ipairs(VipConfig[1].pfNeedYb or {}) do
       if v.needYb == 0 and v.pf == pf then
         if vipLevel < 1 then
            LActor.setVipLevel(actor,1)
            updateVipAttr(actor)
         end
    ...
    if vipLevel < vipStartLevel.startLevel then
        vipLevel = vipStartLevel.startLevel
        LActor.setVipLevel(actor,vipLevel)
        updateVipAttr(actor)
    end
      print("actor vip:".. vipLevel.. " actorid:" .. LActor.getActorId(actor))
    local npack = LDataPack.allocPacket(actor, Protocol.CMD_Vip, Protocol.sVipCmd_InitData)
    if npack == nil then return end
    
    
    LDataPack.writeShort(npack, vipLevel)
...
local function getpfNeedYb(actor, vipLevelConf)
    local pf = LActor.getPf(actor)
    for i,v in ipairs(vipLevelConf.pfNeedYb or {}) do
        if v.pf == pf then
            return v.needYb
        end
    end
    return vipLevelConf.needYb
end
...
local function onCharge(actor, val)
    ...
    while true do
        ...
        local needYb = getpfNeedYb(actor, conf)
        if totalCharge < needYb then break end
        level = level + 1
        update = true
    end
    if update then
        ...
        System.writeLocalLog(qlVip,actor,level)
    end
...
local function onBuyGift(actor, packet)
	...
	--发礼包奖励
	LActor.giveAwards(actor, cfg.awards, "购买VIP礼包")
	--设置已经购买
	-- data.gift = System.bitOpSetMask(data.gift or 0, id, true)
    setExtMask(data, id)
	--返回消息给客户端
...
--测试充值命令
function gmTestRecharge(actor, yb,paytype)
    LActor.addRecharge(actor, yb,  -1 , "",paytype)
end
