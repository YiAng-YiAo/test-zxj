...
local function onReqPromote(actor, packet)
	...
	elseif method == 2 then --普通物品提升
		...
		LActor.consumeItem(actor, zsconfig.normalItem, 1, useyuanbao, "转生")
	...
	System.writeLocalLog(qlCircleExp,actor, exp, method)
...
local function onReqUpgrade(actor, packet)
	...
	System.writeLocalLog(qlCircle,actor, actordata.zhuansheng_lv)
	updateInfo(actor)
...
function gmCircle(actor, v)
	local actordata = LActor.getActorData(actor)
	actordata.zhuansheng_lv = v or 0
	updateInfo(actor)
	actorevent.onEvent(actor, aeZhuansheng, actordata.zhuansheng_lv)
end
