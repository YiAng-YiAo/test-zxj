...
local function sendMonthCardFirstBuyRewardMail(actor)
	local actorVar =  LActor.getStaticVar(actor)
	if actorVar.firstmonthcard then return end
	local actorId = LActor.getActorId(actor)
	local mail_data = {}
...
local function sendPrivilegeOpenRewardMail(actor)
	local actorVar =  LActor.getStaticVar(actor)
	if actorVar.firstprivilege then return end
	local actorId = LActor.getActorId(actor)
	local mail_data = {}
...
function buyMonthCard(actor)
	...
	--月卡首购发奖励
	if 0 == var.endTime then
		local actorVar = LActor.getStaticVar(actor)
		if nil == actorVar.firstmonthcard then 
			sendMonthCardFirstBuyRewardMail(actor)
		end
	end
	...
	--quicklog
	local timelen = (var.endTime or 0) - System.getNowTime()
	System.writeLocalLog(qlMonth,actor, timelen)
	--开通后发第一天的奖励
	if not isOpen then 
		var.lastDay = 0
		var.sendEndMail = 1
...
function buyPrivilege(actor)
	...
	local var = getData(actor)
	local timelen = (var.endTime or 0) - System.getNowTime()
	System.writeLocalLog(qlPrivilegeMonth,actor, timelen)
