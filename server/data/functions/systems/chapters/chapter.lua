...
local auditing  = require("systems.platformactivity.auditing")
...
local function onChallengeBoss(actor, packet)
	...
	local x = 0
	local y = 0
	local sceneCfg = InstanceConfig[conf.bossFid]
	if sceneCfg then
		x = (sceneCfg.bossjrd or {})[1] or 0 
		y = (sceneCfg.bossjrd or {})[2] or 0
	end
	--]]
	--print(LActor.getActorId(actor).."=========chapter.onChallengeBoss x:"..x..",y:"..y.."fbid:"..conf.bossFid)
	local sceneHandle = ins.scene_list[1]
	local sceneId
	if not Fuben.canMove(sceneHandle,x,y) then--客户端坐标判断是否可以移动
		sceneId, x , y = getChapterSceneId(actor, data.level)
		x = x* GRIDSIZE
		y = y* GRIDSIZE
	end
...
local function onReqChapterReward(actor, packet)
	...
	if ret then
		...
		LActor.giveAwards(actor, conf.rewards, "章节奖励")
...
local function onLogin(actor)
	...
	local chapter =  LActor.getChapterLevel(actor) + 1
	--local sceneId, x , y = getChapterSceneId(actor, chapter)
...
local function onBeforeLogin(actor, offlineTime, logoutTime)
	...
	if bagSpace >= rewardCount then
		...
		for _, v in ipairs(reward) do
			LActor.giveAward(actor, v.type, v.id, v.count, "离线奖励")
		...
		for _, v in ipairs(reward) do
			if (v.type or 0) == AwardType_Item then
				...
				if itemConf and item.isEquip(ItemConfig[v.id or 0]) then
					...
					if count > 0 then
						LActor.giveAward(actor, v.type, v.id, count, "离线奖励")
					end
		...
		for _, v in ipairs(left_reward) do --这里全部会是需要背包格子的装备
			if (v.type or 0) == AwardType_Item then
				if left > 0 then --还有剩余的背包格子
					...
					LActor.giveAward(actor, v.type, v.id, gcount, "离线奖励")
	...
	LActor.addExp(actor, var.offlinedata.exp , "离线奖励")
	LActor.changeGold(actor, var.offlinedata.gold , "离线奖励")
...
local function getChapterSceneId(actor,cid)
	local  randChapter = auditing.SetAuditingData(actor)
	if randChapter > 0 then
		cid = randChapter
	end
	...
	local x = cfg.enterPos[1] or 0
	local y = cfg.enterPos[2] or 0
	local var = LActor.getDynamicVar(actor)
	if var.ChallengeBossFlag and var.ChallengeBossX and var.ChallengeBossY and var.ChallengeScene then
		if cfg.sid  == var.ChallengeScene and  var.ChallengeBossX ~= 0 and  var.ChallengeBossY ~= 0  then
			x =  var.ChallengeBossX
			y =  var.ChallengeBossY
		end
		var.ChallengeBossFlag = nil
		var.ChallengeBossX = nil
		var.ChallengeBossY = nil
	end
    return cfg.sid, x, y
...
local function onReqGetReward(actor, packet)
	...
	LActor.giveAwards(actor, allAwards, "关卡杀怪奖励")
end
	...
	if lv == nil then
		...
		if lv ~= nil  and ChaptersConfig[lv] ~= nil then
			...
			if LActor.isInFuben(actor) then
				...
				local sceneId, x , y = getChapterSceneId(actor, data.level)
				local fbHandle = Fuben.getStaticFubenHandle()
				if sceneId == LActor.getSceneId(actor) then
					LActor.reEnterScene(actor,x, y)
				else
					LActor.enterFuBen(actor,fbHandle,sceneId, x ,y)
				end
				onEnterStaticFuben(nil, actor)
