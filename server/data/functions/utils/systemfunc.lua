...
local SYSTEM_VAR_OLD_FILENAME = "sysvar_%d.bin"
local SYSTEM_VAR_FILENAME = "system_var_%d.bin"
...
{
	-- 新版本的文件放在前面
	["CHAT_VAR"]   = { varName="CHAT_VAR",   fileName="system_chat_%d.bin", data=_G.staticChatVar, isLoad=nil },
	["SYSTEM_VAR"] = { varName="SYSTEM_VAR", fileName="system_var_%d.bin",  data=_G.systemStaticVar,   isLoad=nil },
}
...
local function loadConfData(varName)
	...
	local filePathName = string.format("%s/%s",System.getRuntimePath(),fileName)
	if conf.data == nil then
		local loadedData, err = loadData(filePathName)
	...
	if varName == "SYSTEM_VAR" and not next(conf.data) then
		...
		local old_filePathName = string.format("%s/%s",System.getRuntimePath(),old_fileName)
		local ud = System.loadFileAsUserData(old_filePathName)
...
local function saveConfData(varName)
	...
	local fileName = string.format(conf.fileName, System.getServerId())
	local filePathName = string.format("%s/%s",System.getRuntimePath(),fileName)
	saveData(filePathName, conf.data)
end
