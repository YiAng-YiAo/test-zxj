...
MianSrvHash = MianSrvHash or {}
AllSrvHash = AllSrvHash or {}
...
function setCommonSrvList(commSrvList)
	local CommSrvList = getCommonSrvList()
	for i,conf in ipairs(commSrvList) do
		for i=conf.startid, conf.endid do
			if not CommSrvList[i] then
				CommSrvList[i] = 0
			end
		end
	end
...
function StopCrossCon()
	local battleSrvId = GetMainBattleSvrId()
	if battleSrvId > 0 then
		System.delOneGameClient(battleSrvId)
	end
end
-- 加载跨服配置设置服务器flag
